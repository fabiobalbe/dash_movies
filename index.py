        ...
        html.Div(
            ...
            """
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Slider(
                                    ...
                                    marks={
                                        str(yrs): str(yrs)
                                        for yrs in range(year_list[0], year_list[-1], 2)
                                    },
                ...
                """
                        ...
                        html.Div([
                            html.Div(id="text1", className="text_container"),
                            html.Div(id="text2", className="text_container1"),
                            html.Div(id="text3", className="text_container1"),
                            html.Div(id="text4", className="text_container1"),
                            html.Div(id="text5", className="text_container2")
...
def display_data(select_year):
    ...
    df2 = df1[(df1["genre"] == "Musical") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df3 = df1["total_gross"].sum()
    df4 = (df2 / df3) * 100

    df5 = df1[(df1["genre"] == "Comedy") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df6 = (df5 / df3) * 100

    df7 = df1[(df1["genre"] == "Adventure") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df8 = (df7 / df3) * 100

    df9 = df1[(df1["genre"] == "Romantic Comedy") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df10 = (df9 / df3) * 100

    df11 = df1[(df1["genre"] == "Western") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df12 = (df11 / df3) * 100

    df13 = df1[(df1["genre"] == "Action") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df14 = (df13 / df3) * 100

    df15 = df1[(df1["genre"] == "Drama") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df16 = (df15 / df3) * 100

    df17 = df1[(df1["genre"] == "Triller/Suspense") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df18 = (df17 / df3) * 100

    df19 = df1[(df1["genre"] == "Black Comedy") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df20 = (df19 / df3) * 100

    df21 = df1[(df1["genre"] == "Documentary") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df22 = (df21 / df3) * 100

    df23 = df1[(df1["genre"] == "Horror") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df24 = (df23 / df3) * 100

    df25 = df1[(df1["genre"] == "Concert/Performance") & (df1["Year"] == select_year)][
        "total_gross"
    ].sum()
    df26 = (df25 / df3) * 100
...
def display_data(select_year):
    ...
    return {
        ...
        "layout": go.Layout(
            ...
            paper_bgcolor="white",
            title={
                "text": "<b>Gross Sales in " + str(select_year) + "</b>",
                "y": 0.98,
                "x": 0.5,
                "xanchor": "center",
                "yanchor": "top",
...
def display_data(select_year):
    ...
    return {
        ...
        "layout": go.Layout(
            ...
            paper_bgcolor="white",
            title={
                "text": "<b>Sales by Rating in " + str(select_year),
                "y": 0.97,
                "x": 0.5,
                "xanchor": "center",
                "yanchor": "top",
...
@app.callback(Output("text1", "children"), [Input("select_year", "value")])
def display_data(select_year):

    g = df[(df["mpaa_rating"] == "G") & (df["Year"] == select_year)][
        "mpaa_rating"
    ].count()
    p_g = df[(df["mpaa_rating"] == "PG") & (df["Year"] == select_year)][
        "mpaa_rating"
    ].count()
    p_g13 = df[(df["mpaa_rating"] == "PG-13") & (df["Year"] == select_year)][
        "mpaa_rating"
    ].count()
    r = df[(df["mpaa_rating"] == "R") & (df["Year"] == select_year)][
        "mpaa_rating"
    ].count()
    nr = df[(df["mpaa_rating"] == "Not Rated") & (df["Year"] == select_year)][
        "mpaa_rating"
    ].count()
    total_rated = g + p_g + p_g13 + r + nr
                ...
                html.Tbody(
                    [
                        html.Tr(
                            ...
                            html.Td("{0:.2f}%".format(g_per) + ' share in ' + str(select_year), className="style2"),
                            ]
                        )
                    ]
                )
...
@app.callback(Output("text2", "children"), [Input("select_year", "value")])
def display_data(select_year):
                ...
                html.Tbody(
                    [
                        html.Tr(
                            ...
                            html.Td("{0:.2f}%".format(p_g_per) + ' share in ' + str(select_year), className="style2"),
                            ]
                        )
                    ]
                )
...
@app.callback(Output("text3", "children"), [Input("select_year", "value")])
def display_data(select_year):
    ...
    total_rated = g + p_g + p_g13 + r + nr
    p_g_13_per = (p_g13 / total_rated) * 100
            ...
            [
                html.Tbody(
                    [
                        html.Tr(
                            [
                            html.Td("PG13:", className="style1"),
                            html.Td("{0:.2f}%".format(p_g_13_per) + ' share in ' + str(select_year), className="style2"),
                            ]
                        )
                    ]
                )
...
@app.callback(Output("text4", "children"), [Input("select_year", "value")])
def display_data(select_year):
                ...
                html.Tbody(
                    [
                        html.Tr(
                            ...
                            html.Td("{0:.2f}%".format(r_per) + ' share in ' + str(select_year), className="style2"),
                            ]
                        )
                    ]
                )
...
@app.callback(Output("text5", "children"), [Input("select_year", "value")])
def display_data(select_year):
                ...
                html.Tbody(
                    [
                        html.Tr(
                            ...
                            html.Td("{0:.2f}%".format(nr_per) + ' share in ' + str(select_year), className="style2"),
                            ]
                        )
                    ]
                )
